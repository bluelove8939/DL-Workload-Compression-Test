...
class CompressionQuantTestbench(object):
    ...
    def generate_weight_compr_hook(self, model_name, submodule_name, submodule_info) -> callable:
        def weight_compr_hook(submodule, input_tensor, output_tensor):
            ...
            lowered_weight = weight_lowering(weight=submodule.weight, layer_info=submodule_info).detach().cpu().numpy()
...
if __name__ == '__main__':
    ...
    for name, config in imagenet_pretrained.items():
        ...
        model = generate_from_chkpoint(
            ...
            chkpoint_path=os.path.join(os.curdir, 'model_output', f"{name}_pruned_tuned_pamt_0.5.pth"),)
