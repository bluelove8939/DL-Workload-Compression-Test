...
if __name__ == '__main__':
    ...
    result_files = [
        os.path.join(os.curdir, 'logs', 'accelerator_cycles.csv'),
        os.path.join(os.curdir, 'logs', 'accelerator_cycles_quant.csv'),
    ]
    ...
    analyzer.results['performance gain'] = np.array(analyzer.results['total']) / np.array(analyzer.results['cycles'])
    ...
    plt.tight_layout()
    # plt.savefig("G:\내 드라이브\ICEIC 2023\Fig_accelerator_accurate_performance_gain.pdf",
    #             dpi=200, bbox_inches='tight', pad_inches=0)
    # plt.show()

    print(np.average(np.array(analyzer.results['performance gain'])))